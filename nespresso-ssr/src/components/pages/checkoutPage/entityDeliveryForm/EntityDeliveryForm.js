...
import { INN_REGEX } from "../../../../constants/regex";
import PhoneMask from "../../../masks/phoneMask/PhoneMask";
import isSameDayAndAfter6PM from "../../../../utils/isSameDayAndAfter6PM";
import Link from "next/link";

const EntityDeliveryForm = () => {
  const dispatch = useDispatch();

  const formikRef = useRef(null);

  const { status, shippingAddress, shippingAddressesList } = useSelector(
    (state) => state.cart
  );
  ...
  return (
    <Formik
      ...
      initialValues={{
        ...
        mrr: true,
        inn: shippingAddress?.inn ?? "",
        policeConcent: false,
      }}
      validationSchema={Yup.object().shape({
        fullName: Yup.string().max(255).required("Введите имя компании"),
        mobile: Yup.string()
        ...
        email: Yup.string()
          ...
          .required("Пожалуйста, введите адрес электронной почты, компании"),
        mrr: Yup.bool()
          .oneOf([true, false], "Выберите корректное значение")
          .required("Выберите корректное значение"),
        inn: Yup.string()
          .max(12, "Максимальное число цифр в ИНН: 12")
          .required("Введите ИНН")
          .matches(INN_REGEX, "Обнаружен недопустимый символ"),
...
export default EntityDeliveryForm;
