...
import ExclusiveProductPage from "../../src/components/pages/exclusiveProductPage/ExclusiveProductPage";
import getCurrUrl from "../../src/utils/getCurrUrl";

const { getAllExclusiveProductsIds, getExclusiveProduct } = new CoffeeService();

export default function ExclusiveProduct({ singleProduct }) {
  const description =
  ...
  return (
    <>
      ...
      <ExclusiveProductPage product={singleProduct} />
    </>
  );
}

export async function getStaticPaths() {
  const res = await getAllExclusiveProductsIds();
...
export async function getStaticProps({ params }) {
  ...
  try {
    const singleProduct = await getExclusiveProduct(params.type);
